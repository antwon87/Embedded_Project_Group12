...
const double samplingFrequency = 25000; //Hz, must be less than 10000 due to ADC
...
void loop() {
  ...
  while (validSamples < 10) {
    ...
    if (vReal[tarIndex] > 30) {  // 30 chosen arbitrarily. Needs testing.
      validSamples++;
      tarMag += vReal[tarIndex];
    }
  }
  ...
  switch (state) {
    ...
    case SEARCHING:  // Aim car at target beacon if searching
      ...
      if (tarMag > 30) {  // Don't do anything if target magnitude is insignificant, buzzer probably off
...
int freqToIndex(int f) {
  return (f * (samples / 2)) / samplingFrequency;
}
